...
| Open group traffic | Complete sender capture and at least 99% bidirectional delivery with two stable RTL8812AU radios | 0.1.6 initial probes delivered 391/400 then 394/400 when `…08:c1` sent from `1-1.4`; after physically moving it to `1-1.1`, it delivered 333/400, while `…0d:8b` delivered 395/400 from the swapped `1-1.4`. Each measured interval had zero transport event | **Failing test unit/attached antenna; tested USB branches ruled out as primary cause** |
| SAE/AMPE | Both peers prove completed SAE and decrypted AMPE, then pass bidirectional unicast, multicast, HWMP, and checksummed payload | DKMS 0.1.5 passed peer-specific SAE acceptance/decrypted AMPE, MFP/authorization, bidirectional unicast/multicast/HWMP, and 32 MiB checksummed transfer in both directions with zero transport events | Pass for secured smoke on validated topology |
...
| 5 GHz HT20 compatibility | Open peering, HWMP, sender-captured multicast, SAE/AMPE, transfer, and short churn on permitted 5 GHz channels with two RTL8812AU peers | Exact 0.1.6 smoke passed open bilateral peering/HWMP and one serialized churn cycle on non-DFS channel 149 at the DK-enforced 13 dBm limit, with clean kernel interval; quantitative multicast, security, transfer, and multi-cycle coverage remain | Smoke pass; full gate pending |
| 5 GHz HT40 compatibility | Same as 5 GHz HT20 at 40 MHz where permitted, including DFS handling when the selected channel requires it | Exact 0.1.6 smoke passed open bilateral peering/HWMP and one serialized churn cycle on the non-DFS channel-149/153 HT40+ block at the DK-enforced 13 dBm limit; quantitative multicast, security, transfer, multi-cycle, and DFS coverage remain | Smoke pass; full gate pending |

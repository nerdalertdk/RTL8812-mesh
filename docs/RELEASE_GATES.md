...
| Open group traffic | Complete sender capture and at least 99% bidirectional delivery with two stable RTL8812AU radios | DKMS 0.1.5 passed qualitative binary multicast/churn, but four 400-frame sender-captured probes fell below 99%. Physical port swapping points to a shared hub/RF path rather than an adapter identity; a direct or independently powered control is pending | Blocked pending physical-path control |

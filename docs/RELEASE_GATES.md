...
| TX submission/completion errors | Driver-owned aggregate buffers never enter mac80211 status/purge paths; failed submissions release all callback-owned state; completion errors report no false ACK and are observable without blind replay | Source 0.1.5 separates aggregate/original skb ownership; downstream and pinned-mainline series reproduce exact final hashes. Disposable injection requires post-cleanup proof for aggregate rejection and post-status proof for completion failure; live injection has not run | Pending 0.1.5 hardware injection |
| TX teardown safety | Every submitted TX URB is quiesced before skb, mac80211, or driver state is freed | Source 0.1.5 anchors TX URBs and synchronously kills the anchor after draining the producer; test-only selected-device completion delay plus pre/post queued-URB and active-callback markers make the serialized unbind/rebind test deterministic, but it has not run | Pending 0.1.5 teardown test |

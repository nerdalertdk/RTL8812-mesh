...
| Open mesh peering | Repeated fresh joins with both peer links `ESTAB` | Exact DKMS 0.1.6 passed 20/20 serialized lifecycle joins and the final strict soak sustained bilateral peering on `1-1.1`/`1-1.4` | Pass for validated topology |
...
| Cold HWMP discovery | Bidirectional first contact after path/neighbour flush and paths at both peers | Exact DKMS 0.1.6 serialized lifecycle passed 20/20 bidirectional first contacts and reciprocal HWMP paths; the 2.4 GHz sweep passed 13/13 channels | Pass for HT20 validated topology and DK channels 1--13 |
...
| 5 GHz HT40 compatibility | Same as 5 GHz HT20 at 40 MHz where permitted, including DFS handling when the selected channel requires it | Exact 0.1.6 passed five non-DFS channel-149/153 HT40+ churn cycles, width-verified 40 MHz SAE/AMPE, 400/400 bidirectional multicast, 64 MiB open transfers, and protected 32 MiB transfers at the DK-enforced 13 dBm limit, all with clean kernel intervals. DFS remains unavailable because radar detection is not exposed | Pass for non-DFS channel-149/153 HT40+; DFS unavailable |
...
| Endurance | Bounded unattended run with peer/HWMP state, traffic, checksums, recovery, USB events, temperature, and power flags | Exact DKMS 0.1.6 strict 30-minute soak completed 37/37 bilateral `ESTAB` states, 74/74 lossless batches, and 12/12 checksum-verified transfers with zero recovery, invalidation, scoped transport event, or throttle condition on `1-1.1`/`1-1.4`; 73.036--75.958 C | Pass for 0.1.6 bounded validated topology |

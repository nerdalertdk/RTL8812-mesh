...
- A quantitative 5 GHz HT20 control passed 400/400 group frames in both
  directions with the same `…08:c1` unit, USB branch, mesh role, and 0.1.6
  driver. A 2.4 GHz channel-13 control then passed 399/400 and 400/400 with
  the same unit and antenna. The channel-1 loss is thus local RF-environment
  evidence, not evidence of a driver, adapter, antenna, or USB transport
  defect; see
...
- Exact 0.1.6 passed the SAE/AMPE gate with peer-specific SAE acceptance,
  ...
  then provenance-safely restored open HT20; see
  `tests/results/2026-08-12-secure-mesh-0.1.6.md`.
- Exact 0.1.6 passed the open 512 MiB checksum transfer in both directions
  (5.04/7.01 MB/s), with matching SHA-256, reciprocal HWMP, and no transport
  event; see `tests/results/2026-08-12-open-transfer-0.1.6.md`.
...
- Exact 0.1.6 fully qualified non-DFS channel-149 HT20: five serialized churn
  cycles, 400/400 bidirectional multicast, 64 MiB open transfers, and
